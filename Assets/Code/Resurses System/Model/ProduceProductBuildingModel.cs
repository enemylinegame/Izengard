...
{ 
    ...
    public abstract class ProduceProductBuildingModel<T,M,N> : BuildingModel where T:ScriptableObject where M:Product<T> where N:Holder<T>
    {
        ...
        [SerializeField] protected GlobalResurseStock _globalResurseStock;
        ...
        public void ProductProduced(Product<T> product)
        {
            ...
            _globalResurseStock.AddObjectInStock(product.ObjectProduct,product.ProduceValue);
        ...
        public abstract void GetPaidForProducts(GlobalResurseStock stock);
        
        public void CancelProduce(GlobalResurseStock stock)
        {
            if (ProduceWaitProducts!=null && ProduceWaitProducts.Count > 0)
            { 
